...
let clearBackgroundTexture = null;
...
backgroundLoader.load(
    ...
    (texture) => {
        ...
        clearBackgroundTexture = texture;
...
function changeToClearBackground() {

    if (!clearBackgroundTexture) {

        console.warn(
            "[BACKGROUND] background1.png がまだ読み込まれていません"
        );
...
let revivalPreparing = false;
let revivalPressesRemaining = 0;
let revivalFinished = false;

const REVIVAL_PRESS_COUNT = 3;
// =====================================================
// ラスボス吹っ飛び用
// =====================================================
...
function ensureRevivalOverlay() {

    let overlay =
        document.getElementById("revivalOverlay");

    if (overlay) return overlay;

    overlay = document.createElement("div");
    overlay.id = "revivalOverlay";
...
async function startRevivalSequence() {
    ...
    revivalPreparing = true;

    // 復活は全レベル共通で3回
    revivalPressesRemaining = REVIVAL_PRESS_COUNT;

    const overlay = ensureRevivalOverlay();
    ...
    messageText.innerHTML = `
        <div class="revivalMessage revivalFirst">

            <div class="revivalSymbol">✦</div>

            <div class="revivalMainText">
                <span>なかまの あかしが</span>
                <span>ひかった！</span>
            </div>
    ...
    // -------------------------------------------------
    messageText.innerHTML = `
        <div class="revivalMessage">

            <div class="revivalSubText">
                <span>みんなが ちからを</span>
                <span>おくってくれた！</span>
            </div>
    ...
    overlay.classList.add("isGathering");
    ...
    revivalPreparing = false;
    revivalActive = true;

    debugLog("REVIVAL", {
        requiredPresses: revivalPressesRemaining
    });
}


function showRevivalCounter() {

    messageText.innerHTML = `
        <div class="revivalCounterScreen">

            <div class="revivalCounterLabel">
                なかまの ちからで
            </div>

            <div class="revivalCounterNumber">
                <span class="revivalCounterSmall">あと</span>

                <span id="revivalNumber">
                    ${revivalPressesRemaining}
                </span>

                <span class="revivalCounterSmall">かい！</span>
            </div>

            <div class="revivalCounterGuide">
                ボタンを おして！
            </div>
...
function handleRevivalPress() {
    ...
    debugLog("REVIVAL PRESS", {
        remaining:
            revivalPressesRemaining
    });

    if (revivalPressesRemaining <= 0) {
        finishRevival();
    }
}


async function finishRevival() {

    if (revivalFinished) return;
    ...
    ballMaterial.emissiveIntensity = 7;

    ballGlowLight.color.copy(
        revivalColor
    );

    ballGlowLight.intensity = 8;

    messageText.innerHTML = `
        <div class="revivalComplete">

            <div class="revivalCompleteTop">
                みんなの ちからが
            </div>

            <div class="revivalCompleteMain">
                あつまった！
            </div>
